...
export default function Dashboard() {
  const { utilizador, darkMode } = useAuth();
  ...
  async function carregarDados() {
    try {
      ...
      const todosServicos = servicosRes.data;

      const hoje = new Date().toISOString().split('T')[0];
      const pedidosHoje = todosPedidos.filter(p => p.criado_em?.startsWith(hoje));
  ...
  return (
    <div style={{ display: 'flex', gap: 20, fontFamily: "'Inter', sans-serif", fontSize: 13, minHeight: '100%', alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 20 }}>
          {[
            { icon: '👥', label: 'Clientes Cadastrados', value: stats.clientes, sub: '↑ 8 este mês', bg: '#DBEAFE', click: '/clientes' },
            { icon: '🛒', label: 'Pedidos Hoje', value: stats.pedidosHoje, sub: '+12% comparado ao ontem', bg: '#D1FAE5', click: '/pedidos' },
            { icon: '💵', label: 'Faturamento Hoje', value: `${stats.faturamentoHoje.toLocaleString('pt-MZ', { minimumFractionDigits: 2 })} MT`, sub: '+18% comparado ao ontem', bg: '#FEF3C7', click: '/pagamentos' },
            { icon: '📋', label: 'Pedidos Pendentes', value: stats.pendentes, sub: 'Ver detalhes →', bg: '#EDE9FE', click: '/pedidos' },
          ].map(kpi => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: '16px 20px' }}>
            ...
            {/* Gráfico donut + legenda lado a lado */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flexShrink: 0 }}>
                <ResponsiveContainer width={120} height={120}>
                  <PieChart>
                    <Pie
                      data={pedidosStatus.some(s => s.value > 0)
                        ? pedidosStatus
                        : [{ name: 'Sem dados', value: 1, color: '#dbd1d1' }]
                      }
                      cx="50%" cy="50%"
                      innerRadius={33} outerRadius={55}
                      dataKey="value" paddingAngle={pedidosStatus.some(s => s.value > 0) ? 3 : 0}
                    >
                      {(pedidosStatus.some(s => s.value > 0)
                        ? pedidosStatus
                        : [{ color: '#E2E8F0' }]
                      ).map((e, i) => (
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
                {pedidosStatus.map(s => (
                  <div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    </div><div style={{ color: text, fontSize: 11, fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${border}` }}>
                {['ID', 'Cliente', 'Serviços', 'Data', 'Total', 'Status'].map(h => (
            ...
            <tbody>
              {pedidos.length === 0 ? (
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: 20, color: muted }}>Nenhum pedido ainda</td></tr>
              ) : pedidos.map(p => {
                ...
                return (
                  <tr key={p.id} style={{ borderBottom: `1px solid ${border}` }}>
                    ...
                    <td style={{ padding: '9px 8px', color: muted, maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>—</td>
        ...
        <div style={{ textAlign: 'center', fontSize: 11, color: muted, paddingBottom: 16 }}>
          © 2025 SmartPaper - Gestão Inteligente de Impressão e Papelaria. Todos os direitos reservados.
          <span style={{ float: 'right' }}>Versão 1.0.0</span>
        </div>
      </div>

      {/* ── PAINEL DIREITO ── */}
      <div style={{ width: 220, flexShrink: 0, alignSelf: 'flex-start', position: 'sticky', top: 0 }}>
